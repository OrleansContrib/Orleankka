...
namespace Orleankka.Checks
{
    ...
    public class DispatcherFixture
    {
        ...
        public async Task Auto_wires_any_method_which_has_single_argument_according_to_default_naming_conventions()
        {
            ...
            Assert.That(await dispatcher.DispatchResultAsync(target, new OnVoidMessage()), Is.Null);
            Assert.That(await dispatcher.DispatchResultAsync(target, new OnAsyncVoidMessage()), Is.Null);
            Assert.That(await dispatcher.DispatchResultAsync(target, new OnResultMessage()), Is.EqualTo("42"));
            Assert.That(await dispatcher.DispatchResultAsync(target, new OnAsyncResultMessage()), Is.EqualTo("42"));

            Assert.That(await dispatcher.DispatchResultAsync(target, new HandleVoidMessage()), Is.Null);
            Assert.That(await dispatcher.DispatchResultAsync(target, new HandleAsyncVoidMessage()), Is.Null);
            Assert.That(await dispatcher.DispatchResultAsync(target, new HandleResultMessage()), Is.EqualTo("42"));
            Assert.That(await dispatcher.DispatchResultAsync(target, new HandleAsyncResultMessage()), Is.EqualTo("42"));

            Assert.DoesNotThrowAsync(
                async () => await dispatcher.DispatchResultAsync(target, new PrivateHandlerMessage()));

            Assert.DoesNotThrowAsync(
                async () => await dispatcher.DispatchResultAsync(target, new InternalHandlerMessage()));

            Assert.ThrowsAsync<Dispatcher.HandlerNotFoundException>(
                async () => await dispatcher.DispatchResultAsync(target, new NonSingleArgumentHandlerMessage()));

            Assert.ThrowsAsync<Dispatcher.HandlerNotFoundException>(
                async () => await dispatcher.DispatchResultAsync(target, "custom handler name"));
        }
        ...
        public async Task Custom_naming_conventions()
        {
            ...
            Assert.That(await dispatcher.DispatchResultAsync(target, "custom handler name"), Is.EqualTo(42));
        }
